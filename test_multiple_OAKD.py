...
#ourblobfile = "/frozen_inference_graph_2021.4.blob"
#ourblobfile = "/frozen_inference_graph.blob"
#ourblobfile = "/frozen_inference_graph_first_mignon_NN.blob"
#ourblobfile = "/frozen_inference_graph_second_mignon_NN.blob"
#ourblobfile = "/frozen_inference_graph_2500.blob"
ourblobfile = "/frozen_inference_graph_7000-13-02-2022.blob"
nnPathDefault = str((Path(__file__).parent /
                     Path(ourblobpath + ourblobfile)).resolve().absolute())

if not Path(nnPathDefault).exists():
    import sys
    raise FileNotFoundError(
        f'Required file: ' + str(nnPathDefault) +
        ' not found. Create folder and put the blob file there!')

# Labelmap for detector object
#labelMap = [
...
# Labelmap for nn object
#labelMap = ["", "bird", "cat", "dog"]
...
with contextlib.ExitStack() as stack:
    ...
    while True:
        ...
        if cv2.waitKey(1) == ord('s'):
            ...
            #create_labimg_xml(str(images_path),str("capture_"+str(index)),detect_label,width,height,xmin,ymin,xmax,ymax)
